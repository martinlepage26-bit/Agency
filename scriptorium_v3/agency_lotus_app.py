...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Agency LOTUS score app")
    parser.add_argument("--self-test", action="store_true")
    return parser


class AgencyLotusApp(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Agency LOTUS")
        ...
        self.status_var = tk.StringVar(value="Agency LOTUS is ready.")
        self.summary_var = tk.StringVar(value="Upload notes or review the Dr. Sort feed to see agency and creative-meaning scores.")
    ...
    def _build_ui(self) -> None:
        ...
        hero.pack(fill="x")
        ttk.Label(hero, text="Agency LOTUS", style="Header.TLabel").pack(anchor="w")
        ttk.Label(
            hero,
            text="Standalone agency-score and creative-meaning workspace fed by Dr. Sort.",
        ...
        ttk.Button(toolbar, text="Open Dr. Sort", style="Primary.TButton", command=self.open_dr_sort).grid(row=0, column=3)
    ...
    def refresh_notes(self) -> None:
        ...
        self.summary_var.set(
            f"LOTUS has {len(self.notes)} note(s), including {feed_count} Dr. Sort feed item(s). Agency scoring and creative-meaning scoring are local and content-aware."
        )
    ...
    def upload_notes(self) -> None:
        chosen = filedialog.askopenfilenames(
            title="Upload notes to Agency LOTUS",
    ...
    def open_lotus_folder(self) -> None:
        ...
        try:
            ...
            messagebox.showerror("Agency LOTUS", str(exc))

    def open_dr_sort(self) -> None:
        if not self.dr_sort_launcher.exists():
            messagebox.showerror("Agency LOTUS", "Dr. Sort launcher was not found.")
            return
        try:
