def calculate_agency(intake):
    ...
    if any(i["tier"] == "A" and "isolated" in i["value"].lower() for i in signals):
        p_lat = 12.0
        perceptual_caps.append("Tier A Cap: Social Isolation (Limit 12)")
    ...
    if any(i["tier"] == "A" and "high stress" in i["value"].lower() for i in constraints):
        r_band = 8.0
        bandwidth_caps.append("Tier A Cap: Allostatic Overload (Limit 8)")

    total = p_lat + r_band + r_acc + s_leg
    return {
        "agency_total": total,
        "subscores": {
            "perceptual_latitude": {"score": p_lat, "caps_applied": perceptual_caps},
            "regulatory_bandwidth": {"score": r_band, "caps_applied": bandwidth_caps},
